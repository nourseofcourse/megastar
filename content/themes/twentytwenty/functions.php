...
add_role( 'student', __('Student'), array('read' => true, 'edit_posts' => false) );
if( function_exists( 'add_menu_page' ) ) {
	add_menu_page( 'Learning Center', 'Learning Center', 'manage_options', 'learning-center', '', '');
}
...
add_filter( 'rest_prepare_post', function( $response, $post, $request ) {
	...
	$response->data[ 'user_id' ] = $response->data['author'];
...
add_action( 'rest_user_query', 'override_users_per_page', 10, 3 );

function override_users_per_page( $params ) {
	$params[ 'posts_per_page' ] = 200;
	return $params;
}

add_action( 'rest_api_init', function () {
  register_rest_route( 'learningcenter/v1', 'course/(?P<slug>[a-zA-Z0-9-]+)', array(
    'methods' => 'GET',
    'callback' => 'build_course',
  ) );
} );

add_action( 'rest_api_init', function () {
  register_rest_route( 'learningcenter/v1', 'activity', array(
    'methods' => 'GET',
...
function get_user_activity() {
	global $wpdb;
	$results = $wpdb->get_results("SELECT * FROM wp_user_activity");
...
function get_course_lessons( $course_id ) {
	...
	$course_lesson_order = get_post_meta( $course_id, '_lesson_order' );
	...
	if( $course_lesson_order != '' ) {
		$course_lesson_query_args['orderby'] = 'post__in';
		$course_lesson_query_args['post__in'] = preg_split( "/\,/", $course_lesson_order );
...
function get_course_module_order( $course_id = 0 ) {
	if ( $course_id ) {
...
function get_course_modules( $course_id = 0 ) {
	...
	$order = get_course_module_order( $course_id );
...
/**
	 ...
	 */
	function get_lesson_module( $lesson_id = 0 ) {
		...
		$module->url = get_term_link( $module, 'module' );
		...
		if ( isset( $course_id ) && 0 < $course_id ) {
			...
			if ( ! has_term( $module->term_id, 'module', $course_id ) ) {
				return false;
			}
...
function build_course( $data ) {

	$course = get_page_by_path( $data['slug'] , OBJECT, 'course' );
	$course_id = $course->ID;
	$course = get_post( $course_id );
	$lessons = get_course_lessons( $course_id );
	$modules = get_course_modules( $course_id );
	$lessonsWithModules = array();
	

	foreach( $lessons as $lesson ) {
		$module = get_lesson_module( $lesson->ID );
		$lesson->module = $module;
		array_push( $lessonsWithModules, $lesson );
	}
